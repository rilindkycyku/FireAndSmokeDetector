...
void loop() {
  ...
  if (gasSensor > gasSensorThres  && flameSensor==LOW){
   digitalWrite(redLed1, HIGH);
    tone(buzzer1, 9000); //the buzzer sound frequency at 5000 Hz
    digitalWrite(redLed2, HIGH);
    tone(buzzer2, 9000); //the buzzer sound frequency at 5000 Hz
  ...
  else if (flameSensor==LOW && gasSensor < gasSensorThres){ // HIGH MEANS NO FLAME
